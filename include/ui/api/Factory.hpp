...
ui::entity CreateTextEdit(const std::string& placeholder = "", bool multiline = false,
...
ui::entity CreateRadioGroup(const std::vector<std::string>& options, int selectedIndex = 0,
                            std::string_view alias = "");
...
ui::entity CreateDropDown(const std::vector<std::string>& options, int selectedIndex = 0,
                          std::string_view alias = "");
void CloseDropDownPopup(ui::entity ddEntity);

/// 创建列表视图（单选/多选 + 滚动）。items 为数据源；selectedIndex 为初始单选索引（-1 无选中）。
ui::entity CreateListView(const std::vector<std::string>& items, int selectedIndex = -1,
                          std::string_view alias = "");
...
ui::entity SetTooltip(ui::entity target, const std::string& text,
