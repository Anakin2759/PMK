...
inline void CreateMenuDialog()
{
    ...
    // 创建菜单对话框
    auto menuDialog = ui::factory::CreateDialog("VMP-ui Menu", "menuDialog");

    menuDialog | Size(200.0F, 300.0F) | BackgroundColor({0.15F, 0.15F, 0.15F, 0.95F}) |
        BorderRadius(12.0F) | LayoutDirection(ui::policies::LayoutDirection::VERTICAL) | Spacing(15.0F) |
        Padding(20.0F);
    ...
    // 创建开始按钮
    auto startBtnResult = ui::factory::CreateButton("开始", "startBtn");
    if (!startBtnResult)
    {
        ui::log::Error("Failed to create start button: {}", startBtnResult.error().ToString());
        return;
    }
    const auto startBtn = startBtnResult->raw;

    startBtn | buttonStyle | BackgroundColor({0.2F, 0.4F, 0.8F, 1.0F}) |
        BorderColor({0.4F, 0.6F, 1.0F, 1.0F}) | FontSize(14.0F) | BorderRadius(10.0F) |
        OnClick([menuDialog]()
                {
                    CreateMainWindow();
    ...
    // 创建设置按钮
    auto settingsBtnResult = ui::factory::CreateButton("设置", "settingsBtn");
    if (!settingsBtnResult)
    {
        ui::log::Error("Failed to create settings button: {}", settingsBtnResult.error().ToString());
        return;
    }
    const auto settingsBtn = settingsBtnResult->raw;

    settingsBtn | buttonStyle | TextColor({1.0F, 1.0F, 1.0F, 1.0F}) |
        BackgroundColor({0.3F, 0.3F, 0.3F, 1.0F}) | BorderColor({0.5F, 0.5F, 0.5F, 1.0F}) |
        FontSize(14.0F);

    menuDialog | AddChild(settingsBtn);

    // 创建退出按钮
    auto exitBtnResult = ui::factory::CreateButton("退出", "exitBtn");
    if (!exitBtnResult)
    {
        ui::log::Error("Failed to create exit button: {}", exitBtnResult.error().ToString());
        return;
    }
    const auto exitBtn = exitBtnResult->raw;

    exitBtn | buttonStyle | BackgroundColor({0.6F, 0.2F, 0.2F, 1.0F}) |
        BorderColor({0.8F, 0.3F, 0.3F, 1.0F}) | FontSize(14.0F) |
        OnClick([]()
                {
                    ui::log::Info("退出menu.");
