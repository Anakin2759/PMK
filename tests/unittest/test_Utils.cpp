...
{
    const auto entity = factory::CreateLabel(runtime(), "L", "util_label_1");
    auto& registry = ActiveRegistry();
    registry.remove<components::LayoutDirtyTag>(entity);

    utils::MarkLayoutChanged(runtime(), entity);
...
{
    const auto rootLayout = factory::CreateVBoxLayout(runtime(), "gp_layout");
    const auto nestedLayout = factory::CreateVBoxLayout(runtime(), "p_layout");
    const auto leafLabel = factory::CreateLabel(runtime(), "C", "child_layout");

    hierarchy::AddChild(runtime(), rootLayout, nestedLayout);
    hierarchy::AddChild(runtime(), nestedLayout, leafLabel);
    ...
    utils::MarkLayoutChanged(runtime(), leafLabel);
...
{
    const auto parent = factory::CreateVBoxLayout(runtime(), "sib_parent");
    const auto child1 = factory::CreateLabel(runtime(), "C1", "child1_layout");
    const auto child2 = factory::CreateLabel(runtime(), "C2", "child2_layout");

    hierarchy::AddChild(runtime(), parent, child1);
    hierarchy::AddChild(runtime(), parent, child2);
    ...
    utils::MarkLayoutChanged(runtime(), child1);
...
{
    EXPECT_NO_FATAL_FAILURE(utils::MarkLayoutChanged(runtime(), entt::null));
}
...
{
    const auto entity = factory::CreateLabel(runtime(), "V", "util_vis_1");
    auto& registry = ActiveRegistry();
    registry.remove<components::RenderDirtyTag>(entity);

    utils::MarkVisualChanged(runtime(), entity);
...
TEST_F(UtilsTest, MarkVisualChangedAlsoTagsAncestorWindowEntity)
{
    ...
    const auto child = factory::CreateLabel(runtime(), "CW", "child_under_window");
    hierarchy::AddChild(runtime(), windowEntity, child);

    registry.remove<components::RenderDirtyTag>(windowEntity);
    registry.remove<components::RenderDirtyTag>(child);

    utils::MarkVisualChanged(runtime(), child);
...
{
    const auto entity = factory::CreateLabel(runtime(), "Alone", "util_vis_alone");
    auto& registry = ActiveRegistry();
    registry.remove<components::RenderDirtyTag>(entity);
...
{
    EXPECT_NO_FATAL_FAILURE(utils::MarkVisualChanged(runtime(), entt::null));
}
...
{
    const auto entity = factory::CreateLabel(runtime(), "Both", "util_both_dirty");
    auto& registry = ActiveRegistry();
    registry.remove<components::LayoutDirtyTag>(entity);
    registry.remove<components::RenderDirtyTag>(entity);
...
TEST_F(UtilsTest, VerticalScrollbarGeometryDefaultsToInvisibleWithoutScrollArea)
{
    ...
    const auto geometry = utils::GetVerticalScrollbarGeometry(runtime(), entity);
...
TEST_F(UtilsTest, VerticalScrollbarGeometryDefaultsToInvisibleForHorizontalScroll)
{
    ...
    EXPECT_FALSE(utils::GetVerticalScrollbarGeometry(runtime(), entity).visible);
}
...
{
    factory::CreateLabel(runtime(), "Exists", "exist_check_alias");

    EXPECT_TRUE(utils::IsEntityExist(runtime(), "exist_check_alias"));
}

TEST_F(UtilsTest, IsEntityExistReturnsFalseForUnknownAlias)
{
    EXPECT_FALSE(utils::IsEntityExist(runtime(), "totally_unknown_alias_xyz"));
}

TEST_F(UtilsTest, IsEntityExistReturnsFalseAfterRegistryClear)
{
    factory::CreateLabel(runtime(), "Gone", "gone_alias");
    ActiveRegistry().clear();

    EXPECT_FALSE(utils::IsEntityExist(runtime(), "gone_alias"));
}
