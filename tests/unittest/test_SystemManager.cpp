...
public:
    TestSystem(Dispatcher& dispatcher,
               interface::SystemPhase phase,
               std::vector<interface::SystemPhase>& registrationOrder,
               TestSystemCounters& counters)
        : m_dispatcher(&dispatcher),
          m_phase(phase),
          m_registrationOrder(&registrationOrder),
                    m_counters(&counters)
    {}
    ...
    void onEvent([[maybe_unused]] const SystemManagerTestEvent& event) const { ++m_counters->events; }
    [[nodiscard]] interface::SystemPhase getPhase() const noexcept { return m_phase; }
...
TestSystem MakeSystem(UiRuntime& runtime,
                      interface::SystemPhase phase,
                      std::vector<interface::SystemPhase>& registrationOrder,
                      TestSystemCounters& counters)
{
...
TEST(SystemManagerTest, RegistersByPhaseAndLifecycleOperationsAreIdempotent)
{
    ...
    ASSERT_TRUE(manager.addSystemBeforeRegister(MakeSystem(runtime,
                                                          interface::SystemPhase::RENDER,
                                                          order,
                                                          counters)));
    ASSERT_TRUE(manager.addSystemBeforeRegister(MakeSystem(runtime,
                                                          interface::SystemPhase::INPUT,
                                                          order,
                                                          counters)));
    ASSERT_TRUE(manager.addSystemBeforeRegister(MakeSystem(runtime,
                                                          interface::SystemPhase::LAYOUT,
                                                          order,
                                                          counters)));
    ...
    EXPECT_EQ(order, (std::vector{interface::SystemPhase::INPUT,
                                  interface::SystemPhase::LAYOUT,
                                  interface::SystemPhase::RENDER}));
...
TEST(SystemManagerTest, RejectsSystemAdditionAfterRegistrationAndAfterStop)
{
    ...
    manager.registerAllHandlers();
    EXPECT_FALSE(manager.addSystem(MakeSystem(runtime,
                                             interface::SystemPhase::LOGIC,
                                             order,
                                             counters)));

    manager.unregisterAllHandlers();
    EXPECT_FALSE(manager.addSystemBeforeRegister(MakeSystem(runtime,
                                                           interface::SystemPhase::LOGIC,
                                                           order,
                                                           counters)));
    EXPECT_EQ(manager.getSystemCount(), 0U);
