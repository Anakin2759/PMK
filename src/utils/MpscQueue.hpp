 ...
 public:
	explicit MpscQueue(std::size_t capacity)
			: kCapacity(NormalizeCapacity(capacity)),
				kMask(kCapacity - 1),
				buffer_(std::make_unique<Cell[]>(// NOLINT(modernize-avoid-c-arrays,cppcoreguidelines-avoid-c-arrays,hicpp-avoid-c-arrays)
	...
	[[nodiscard]] std::optional<T> TryDequeue() {
		...
		const std::size_t kSequence = cell.sequence.load(std::memory_order_acquire);
		const auto kDifference =
				static_cast<std::intptr_t>(kSequence) - static_cast<std::intptr_t>(kPosition + 1);
 ...
 private:
	...
	struct Cell {
		...
		[[nodiscard]] T* StoragePtr() noexcept {
			return std::launder(reinterpret_cast<T*>(storage.data()));  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
		}
	};
